...
export default function AdhocForm({ isMobile, setActiveForm, visitorToEdit }) {
  ...
  useEffect(() => {
    if (currentAccount) {
      ...
      setVisitors((prevVisitors) =>
        prevVisitors.map((v) => ({
          ...v,
          submittedBy: updatedEmail,
          host: v.onBehalfOf ? v.host : updatedName,
        }))
      );
    }
  }, [currentAccount]);
  ...
  }, [visitorToEdit, ssoEmail, ssoUserName]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!validate()) {
      Swal.fire({ icon: "error", title: "Missing Required Fields", text: "Please fix the errors before submitting." });
      return;
    }
    setLoading(true);
    try {
      ...
      if (visitorToEdit) {
        await axios.put(`${process.env.REACT_APP_API_URL}/api/adhoc/${visitorToEdit._id}`, payload[0]);
        Swal.fire({ icon: "success", title: "Adhoc Visitor Updated!", showConfirmButton: false, timer: 2000 });
      } else {
        await axios.post(`${process.env.REACT_APP_API_URL}/api/adhoc`, payload);
        Swal.fire({ icon: "success", title: "Adhoc Visitors Submitted!", showConfirmButton: false, timer: 2000 });
      }
        ...
        {visitors.map((visitor, index) => (
          ...
          >
            <div className="d-flex justify-content-between align-items-center mb-3">
              ...
              >
                <FaUser className="me-2 text-primary" /> Adhoc Visitor {index + 1}
